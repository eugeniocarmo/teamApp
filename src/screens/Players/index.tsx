...
export function Players() {
  ...
  async function handleAddPlayer() {
    if (newPlayerName.trim().length === 0){
      return Alert.alert("New Player","Don't leave the field empty. Add a new player's name.");
    }
    ...
    try {
      ...
      if (error instanceof AppError){
        ...
        console.log(error);
        Alert.alert('New player', 'Unable to add new player');
      }
    }
  }
  ...
  async function fetchPlayersByTeam() {
    try {
      ...
      console.log(error);
      Alert.alert('Players', 'Unable to fetch players');
    } finally {
      setIsLoading(false);
    }
  }
